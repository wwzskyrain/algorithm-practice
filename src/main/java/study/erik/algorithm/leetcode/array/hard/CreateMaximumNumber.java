...
/**
 * @author yueyi
 * @version : CreateMaximumNumber.java, v 0.1 2022年01月10日 9:47 上午 yueyi Exp $
 */
public class CreateMaximumNumber {

    @LetCodeCommit(title = "321. Create Maximum Number")
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        int[] best = new int[0];
        for (int i = Math.max(0, k - nums2.length); i <= Math.min(k, nums1.length); ++i) {
            best = max(best, 0, maxNumber(maxNumber(nums1, i),
